...
import com.mjminnovations.transactions.*;

public class MJM_Sale {
	...
	private final Integer TransactionType = 1;
	...
	public String SendSaleRequest() {
		...
		try {
			...
			Element requestchild2 = doc.createElement("Amount2");
			request.appendChild(requestchild2);
			Text requestchild2Text = doc.createTextNode(String.valueOf(Amount2));
			requestchild2.appendChild(requestchild2Text);
			...
			// //////// Creating a String representation of the Xml //////////////////
			...
			for (IMessageListener list : AVL_Service.msg_listeners.values())
				list.exception("[exception in SendSaleRequest in MJM_Sale][SendSaleRequest][" + e.getLocalizedMessage() + "] ");
		}
